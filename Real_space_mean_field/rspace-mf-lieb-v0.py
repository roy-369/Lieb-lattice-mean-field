...
U = 0.3			# array for interaction
lamda = 0.0		# array for SOC
...
# given p1, p2, can you tell what rijs enter?
...
ctr= 0
while(1 < 2):
#	print(diff, classicalEn(mfin), netMag(mfin))
	mfout = MFExpValue(mfin)
	diff = DeltaConf(mfin, mfout)

	print(ctr,diff, classicalEn(mfin), np.round(netMag(mfin),2), np.sum(netDen(mfin)))

#	print(classicalEn(mfout))
#	print(netMag(mfout))
#	print(diff, classicalEn(mfout), netMag(mfout))
	mfin = np.multiply(mfout, 1.0-alpha) + np.multiply(mfin, alpha)
	ctr += 1
	if (diff < 1.e-4):
		break

print(np.sum(netDen(mfin)))
...
#soln = optimize.broyden1(MFExpValue, seed)

#print(np.real(netDen(soln)))
